...
def preference_loss(
  ...
  if ipo:
    ...
    losses = -F.logsigmoid(beta * logits) * (
        1 - label_smoothing) - F.logsigmoid(-beta * logits) * label_smoothing
...
def concatenated_inputs(
    batch: Dict[str, Union[List,
                           torch.LongTensor]]) -> Dict[str, torch.LongTensor]:
...
class BasicTrainer(object):
  ...
  def get_batch_samples(self, batch: Dict[str,
                                          torch.LongTensor]) -> Tuple[str, str]:
    ...
    policy_output = all_gather_if_needed(policy_output, self.rank,
                                         self.world_size)
    ...
    if self.config.loss.name in {'dpo', 'ipo'}:
      reference_output = pad_to_length(reference_output, self.config.max_length,
                                       self.tokenizer.pad_token_id)
      reference_output = all_gather_if_needed(reference_output, self.rank,
                                              self.world_size)
  ...
  def get_batch_metrics(self,
    ...
    if loss_config.name in {'dpo', 'ipo'}:
      ...
      chosen_rewards = all_gather_if_needed(chosen_rewards, self.rank,
                                            self.world_size)
      rejected_rewards = all_gather_if_needed(rejected_rewards, self.rank,
                                              self.world_size)
      reward_accuracies = all_gather_if_needed(reward_accuracies, self.rank,
                                               self.world_size)

      metrics[f'rewards_{train_test}/chosen'] = chosen_rewards.cpu().numpy(
      ).tolist()
      metrics[f'rewards_{train_test}/rejected'] = rejected_rewards.cpu().numpy(
      ).tolist()
      ...
      metrics[f'logps_{train_test}/rejected'] = policy_rejected_logps.cpu(
      ).numpy().tolist()

    elif loss_config.name == 'sft':
      policy_chosen_logits = self.policy(
          batch['chosen_input_ids'],
          attention_mask=batch['chosen_attention_mask']).logits.to(
              torch.float32)
    ...
    policy_chosen_logps = all_gather_if_needed(policy_chosen_logps.detach(),
                                               self.rank, self.world_size)
    metrics[f'logps_{train_test}/chosen'] = policy_chosen_logps.cpu().numpy(
    ).tolist()

    all_devices_losses = all_gather_if_needed(losses.detach(), self.rank,
                                              self.world_size)
    ...
    for batch in self.train_iterator:
      ...
      if self.example_counter % self.config.eval_every == 0 and (
        ...
        for eval_batch in (tqdm.tqdm(
            self.eval_batches, desc='Computing eval metrics')
        ...
        if self.config.sample_during_eval:
          ...
          for eval_batch in (tqdm.tqdm(
              sample_batches, desc='Generating samples...')
        ...
        if self.example_counter > 0:
          if self.config.debug:
            ...
            output_dir = os.path.join(self.run_dir,
                                      f'step-{self.example_counter}')
      ...
      for microbatch_idx in range(self.config.gradient_accumulation_steps):
        global_microbatch = slice_and_move_batch_for_device(
            batch, microbatch_idx, self.config.gradient_accumulation_steps,
            self.rank)
      ...
      if last_log is None or time.time(
      ) - last_log > self.config.minimum_log_interval_secs:
        mean_train_metrics = {
